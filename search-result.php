...
//error in case record not found 
	...
	@$ssrtype= $_GET["SSRtype"];
	@$ssr= $_GET["SSR"];
	@$sizeoption=$_GET["sizeoption"];
	@$size=$_GET["size"];
	@$size2=$_GET["size2"];
	...
	foreach ($fields as $field) {	//adding each field to where clause
  		if (isset($_GET[$field]) AND !empty($_GET[$field])) {	//check if field is set and filled
	    	$value = $_GET[$field];
	    	if($field=="size")
	    		if($sizeoption!="BETWEEN")
		    		$sql=$sql." AND `".$field."` ".$sizeoption." '".$value."'";
				else if(isset($_GET["size2"]) AND !empty($_GET["size2"]))
